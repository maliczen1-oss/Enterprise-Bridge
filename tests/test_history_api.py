...
async def test_history_invalid_dates(client, mock_manager_connected):
    resp = await client.get("/api/history?start=notadate&end=alsonot", headers={"Authorization": "Bearer test-token"})
...
async def test_history_success(client, mock_manager_connected):
    ...
    resp = await client.get(f"/api/history?start={start}&end={end}", headers={"Authorization": "Bearer test-token"})
